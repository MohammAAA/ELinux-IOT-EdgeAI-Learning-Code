...
/*
 * 10_hello_cdev_with_ioctl_and_automated_mknod.c - This is the same as (08_hello_cdev_with_ioctl.c)
 * with the addition of the sysfs automatic maniuplation.
 *
 * Behaviour:
 ...
 *   - Device class: class_create(), class_destroy()
 *   - Device instance: device_create(), device_destroy()
 *   - Reading error codes from pointers: IS_ERR(), PTR_ERR()
 */
...
struct hello_device {
    ...
    /* NEW for today: device class and device pointer */
    struct class *cls;           /* device class (/sys/class/hello_cdev) */
    struct device *dev;          /* device instance (/sys/class/hello_cdev/hello_cdevN) */
...
/* Global pointer for cleanup — needed because init/exit don't share
 * a local variable. Yes, a global is acceptable here because:
 *   (1) Only one instance exists (singleton module)
 *   (2) Protected by module reference counting (can't rmmod while open)
 *   (3) Only written during init/exit (not during runtime)
 *   (4) This is the kernel idiom for this exact situation
 */
static struct hello_device *hello_device_ptr;
...
static int __init hello_cdev_init(void)
{
     ...
     *   Dynamic allocation is the standard pattern for drivers (not completely in embedded Linux but I will move on for now).
    ...
    atomic_set(&hello_dev->write_count, 0);
    atomic_set(&hello_dev->ioctl_count, 0);
    ...
    /* =========================================================
     * NEW — Step 4: Create device class
     *
     * This creates /sys/class/hello_cdev/
     * ========================================================= */
    hello_dev->cls = class_create(DEVICE_NAME);
    if (IS_ERR(hello_dev->cls)) {
        pr_err("class_create failed: %ld\n", PTR_ERR(hello_dev->cls));
        ret = PTR_ERR(hello_dev->cls);
        goto fail_class_create;
    }

    /* =========================================================
     * NEW — Step 5: Create device instance
     *
     * This creates /sys/class/hello_cdev/hello_cdev0/
     * and sends a KOBJ_ADD uevent → udev creates /dev/hello_cdev0
     *
     * Parameters:
     *   - cls:    our device class
     *   - parent: NULL (virtual device, no physical parent)
     *   - devt:   our major:minor number
     *   - drvdata: NULL (no extra driver data; use filp->private_data instead)
     *   - fmt:    device name format string
     * ========================================================= */
    hello_dev->dev = device_create(hello_dev->cls, NULL, hello_dev->dev_num,
                             NULL, "%s%d", DEVICE_NAME, MINOR(dev));
    if (IS_ERR(hello_dev->dev)) {
        pr_err("device_create failed: %ld\n", PTR_ERR(hello_dev->dev));
        ret = PTR_ERR(hello_dev->dev);
        goto fail_device_create;
    }

    pr_info("device node auto-created: /dev/%s%d\n",
            DEVICE_NAME, MINOR(dev));

    /* Store hello_dev pointer globally for cleanup (needed if using static alloc,
     * but since we used kzalloc, we'll use the pattern below) */
    hello_device_ptr = hello_dev;
    return 0;
     ...
     */
fail_device_create:
    class_destroy(hello_dev->cls);          /* undo step 5 */

fail_class_create:
    cdev_del(&hello_dev->cdev);             /* undo step 4 */
fail_cdev_add:
...
static void __exit hello_cdev_exit(void)
{
     ...
     */
    /* Step 5 undo: remove device from sys/class/hello_cdev/ */
    device_destroy(hello_dev->cls, hello_dev->dev_num);
    /* Step 4 undo: remove class from sys/class/ */      
    class_destroy(hello_dev->cls);                     /* undo step 3 */
